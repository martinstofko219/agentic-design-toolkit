...
**Implementation** — how the documented design maps to code, for any component including Material-based ones: which input, class, or token realizes each variant and state (e.g. the filled variant is `appearance="filled"`), the key inputs, outputs, and events an integrator actually uses, and gotchas that aren't obvious from the API alone. Derive it from the component's source and the base library's documentation — the grounding rules above apply, so every symbol and mapping is copied verbatim, never inferred. Keep it a map, not a full API reference; when the content outgrows a section, move the depth to a separate implementation page (below) and leave a summary here that points to it.
...
A component's implementation coverage can outgrow the spine section: a full API reference, coding guidelines, integration patterns, form-control behavior, content-projection slots. When it does — judged by volume and depth, not by whether the component is custom — move the deep content to a separate implementation page, its own MDX file bound to the same stories with `<Meta of={...} />` so it appears as a sibling Docs entry, and keep the spine's Implementation section as a summary pointing to it. In practice this is usually custom components; a Material-based component's deep API already lives in the upstream Angular Material docs the page links to, so it earns a full page only when the house wrapper meaningfully diverges from upstream.
...
- [ ] The Implementation section's symbols and variant-to-input mappings are copied verbatim from the component source
